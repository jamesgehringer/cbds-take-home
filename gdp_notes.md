...
- Only joined the raw data, so gdp of every 5 years to match the enrollment data
    - I thought this would give the most accuracte representation
- Plotted using the scatter plot, but tried to plot with GDP as the size of the point
    - This didn't quite give the correct relationship requested by the client, but did highlight some of the bigger GDP players, the role the year plays, and that there may be groups in the data
    - Based on my world map plot of the enrollement data, I think that the region or income group may play a role

![alt text](results/gdp_vs_enrollement_scatter_gdp_scaled_points.png)

- Plotting just gdp vs enrollement doesn't quite show a clear global trend, may be a trend of time. But it does look like there are some distinct groups 
